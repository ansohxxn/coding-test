...
struct Pos { int r, c, dist, use_knight; };
...
int main() {
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (visited[nextR][nextC][now.use_knight]) continue;

			visited[nextR][nextC][now.use_knight] = true;
			q.push({ nextR, nextC, now.dist + 1, now.use_knight });
		}
		...
		for (int i = 0; i < 8; ++i) {
			...
			if (nextR < 0 || nextR >= R || nextC < 0 || nextC >= C) continue;
			if (board[nextR][nextC] == 1) continue;
			if (visited[nextR][nextC][now.use_knight + 1]) continue;

			visited[nextR][nextC][now.use_knight + 1] = true;
			q.push({ nextR, nextC, now.dist + 1, now.use_knight + 1 });
		}
	}
